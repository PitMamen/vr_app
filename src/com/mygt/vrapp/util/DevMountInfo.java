	...
	private void executeInit(Context context) {
		...
		if (mStorageManager != null) {
			...
			try {
				...
				mGetVolumeStateMethod = mStorageManager.getClass().getMethod(METHOD_GET_VOLUME_STATE,
				...
				if (mStorageVolumeList != null && mStorageVolumeList.length > 0) {
					...
					for (int i = 0; i < mStorageVolumeCount; ++i) {
						...
						mIsRemovable = ((Boolean) mIsRemovableMethod.invoke(mStorageVolumeList[i], new Object[0]))
								.booleanValue();
						if (!TextUtils.isEmpty(mStoragePath)) {
							String state = (String) mGetVolumeStateMethod.invoke(mStorageManager,
									new Object[] { mStoragePath });
