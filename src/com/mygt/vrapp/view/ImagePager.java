...
//			iv.setImageResource(resIds[position]);
			AdvertisVo advertisVo = mList.get(position);
			Glide.with(getContext()).load(advertisVo.getUrl()).fitCenter().into(iv);
	...
	public void setViews(List<AdvertisVo> position) {
		this.mList = position;
		setAdapter(adapter);
	}
